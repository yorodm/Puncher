...
if %DEMO_ARG% == "demo" (
  echo Making Puncher Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Puncher Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Puncher-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild Puncher.sln /t:Puncher-app;Puncher-vst3;Puncher-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild Puncher.sln /t:Puncher-app;Puncher-vst3;Puncher-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Puncher.aaxplugin\Contents\Win32\Puncher.aaxplugin --out .\build-win\aax\bin\Puncher.aaxplugin\Contents\Win32\Puncher.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Puncher.aaxplugin\Contents\x64\Puncher.aaxplugin --out .\build-win\aax\bin\Puncher.aaxplugin\Contents\x64\Puncher.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Puncher.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Puncher.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Puncher Installer" ".\installer\Puncher Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Puncher Installer.exe" ".\installer\Puncher Demo Installer.exe"
  REM -del ".\installer\Puncher Installer.exe"
  REM -)
