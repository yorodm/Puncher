...
def main():
  ...
  vst3_bundle = build_dir + "\\Puncher.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\Puncher Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\Puncher Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\Puncher manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\Puncher.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "Puncher.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\Puncher_x64.exe",
      projectpath + "\\build-win\\Puncher_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\Puncher_x64.clap", "Puncher_x64.clap"),
      (projectpath + "\\build-win\\Puncher_ARM64EC.clap", "Puncher_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\Puncher.clap", "Puncher_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\Puncher.clap", "Puncher_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Puncher-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Puncher-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Puncher-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Puncher-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Puncher-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Puncher-clap_ARM64EC.pdb"
  ]
